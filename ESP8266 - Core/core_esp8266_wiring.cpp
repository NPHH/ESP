...
static os_timer_t MicrosOverflowTimer;
static uint32_t MicrosAtLastOverflowTick = 0;
...
void MicrosOverflowTick(void* arg) {
    (void) arg;
    uint32_t m = system_get_time();
    if(m < MicrosAtLastOverflowTick)
        ++MicrosOverflowCount;
    MicrosAtLastOverflowTick = m;
}
...
unsigned long IRAM_ATTR millis()
{
  ...
  uint32_t  m = system_get_time();
  uint32_t  c = MicrosOverflowCount +
                   ((m < MicrosAtLastOverflowTick) ? 1 : 0);
...
uint64_t IRAM_ATTR Micros64() {
    ...
    uint32_t high32_us = MicrosOverflowCount + ((low32_us < MicrosAtLastOverflowTick) ? 1 : 0);
...
void init() {
    initPins();
    timer1_isr_init();
    os_timer_setfn(&MicrosOverflowTimer, (os_timer_func_t*) &MicrosOverflowTick, 0);
    os_timer_arm(&MicrosOverflowTimer, 60000, REPEAT);
}
